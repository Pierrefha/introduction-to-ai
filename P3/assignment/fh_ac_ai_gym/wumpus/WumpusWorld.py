import random as r
import numpy
from fh_ac_ai_gym.wumpus.WorldState import (Perception, World_State, Action,
                                            Direction, Location)
from fh_ac_ai_gym.wumpus.inference_algorithm import Resolution, ForwardChaining
# from fh_ac_ai_gym.wumpus.hunt_the_wumpus import (
#        INFERENCE_ALGORITHM_RESOLUTION, INFERENCE_ALGORITHM_FORWARD_CHAINING)
# todo maybe creat globals file
...
class Wumpus_World:
    ...
    def __init__(self, inference_algorithm):
        ...
        self.init_inference_algorithm(inference_algorithm)
        self.update_perception()

    def init_inference_algorithm(self, inference_algorithm):
        if (inference_algorithm == INFERENCE_ALGORITHM_RESOLUTION):
            self.inference_algorithm = Resolution()
        elif (inference_algorithm == INFERENCE_ALGORITHM_FORWARD_CHAINING):
            self.inference_algorithm = ForwardChaining()
        else:
            print("The inference algorithm you want to use is not supported!")
    ...
    def exec_action(self, action):
        ...
        if next((True for p_l in self.state.pit_locations if
                 self.on_field(a_l, p_l)), False):
            self.state.agent_alive = False
            self.score -= 999
            return False
        ...
        if action == Action.GRAB:
            if (not self.state.has_gold
                    and self.on_field(a_l, self.state.gold_location)):
        ...
        if action == Action.CLIMB:
            if self.state.has_gold:
                # CORRECTED 0,0 to gold location
                if self.on_field(a_l, self.state.gold_location):
                    self.score += 1001
                    return False
        elif (action == Action.ASK):
            query = self.inference_algorithm.get_input_query()
            self.inference_algorithm.ask(query)
        elif action == Action.TELL:
            query = self.inference_algorithm.get_input_query()
            self.inference_algorithm.tell(query)
    ...
    def turn(self, action):
        if action == Action.TURNLEFT:
            self.state.agent_direction = Direction(
                    (self.state.agent_direction.value - 1) % len(Direction))
    ...
    def update_perception(self):
        ...
        if not self.state.agent_alive:
            print("You have died. This game is lost.")
            exit(0)
        ...
        if next((True for p_l in self.state.pit_locations if
                 self.adjacent(a_l, p_l)), False):
            self.percept.breeze = True

        # debug here
        self.inference_algorithm.deduct_rules_based_on_sensory_inputs(self.percept, a_l)
        self.inference_algorithm.knowledge_base.print_kb()
    ...
    def print(self):
        ...
        for y in range(size-1, -1, -1):
            ...
            for x in range(size):
                if self.on_field(self.state.wumpus_location, Location(x, y)):
                    ...
                    print(' ', end='')
                if next((True for p_l in self.state.pit_locations if
                         self.on_field(Location(x, y), p_l)), False):
                    print('P', end='')
                else:
                    print(' ', end='')
                if not self.state.has_gold and (
                        self.on_field(Location(x, y),
                                      self.state.gold_location)):
                    print('G', end='')
                else:
                    print(' ', end='')
                print('|', end='')
        ...
        print('Perception [ St: {}, Br: {}, '.format(self.percept.stench,
              self.percept.breeze), end='')
        print('G: {}, Bu: {}, Sc: {} ]'.format(self.percept.glitter,
              self.percept.bump, self.percept.scream))
        print("Possible actions:\n"
              "Walk: K \t Turn left: J \t Turn right: L\n"
              "Grab gold: G \t Shoot arrow: S  Climb out of pit: C\n"
              "Ask: A \t\t Tell: T")
